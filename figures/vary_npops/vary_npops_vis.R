...
library(extrafont)
setwd("~/MetapopulationDynamics/vary_npops/")
loadfonts()
...
plt =read.csv('output.csv') %>%
  ...
  facet_wrap(. ~ (np_facet), ncol=2, labeller = as_labeller(string_to_tex, default=label_parsed))+
...
plt
ggsave('vary_npops.png', plt, device = png(), height = 10, width=10)
