...
setwd("~/MetapopulationDynamics/abundance_pcc_dif/")
...
plt = read.csv('output.csv') %>%
  ...
  facet_grid(vars(npfacet), vars(m_facet),labeller = as_labeller(string_to_tex, default=label_parsed)) +
...
ggsave("abundance_num_pops.png",  device=png(), plot=plt,  width=10, height=10,)
